...
<form method="GET" action="{{ route('admin.trips.index') }}" class="flex flex-wrap gap-3 items-end px-1 py-3" data-table-filter>
    ...
    @if($selectedProvince)
    <input type="hidden" name="filter[province_id]" value="{{ $selectedProvince->id }}">
    @endif
    ...
    <a href="{{ route('admin.trips.index', ['section' => 'trips', 'tab' => $activeTab] + ($selectedProvince ? ['filter' => ['province_id' => $selectedProvince->id]] : [])) }}"
