...
>
    ...
    <x-admin::table.table>
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody class="divide-y divide-gray-100 bg-white" id="buses-sortable">
                ...
                @empty
                <tr>
                    <td colspan="8" class="px-4 py-10 text-center text-gray-400 text-sm">Chưa có xe nào.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
document.addEventListener('DOMContentLoaded', function () {
    var el = document.getElementById('buses-sortable');
    if (!el || typeof Sortable === 'undefined') return;
    Sortable.create(el, {
        animation: 150,
        onEnd: function () {
            var ids = Array.from(el.querySelectorAll('tr[data-id]')).map(r => r.dataset.id);
            fetch('{{ route('admin.buses.reorder') }}', {
                method: 'POST',
