<x-client.layout :web-profile="$web_profile ?? null" :main-menu="$mainMenu ?? []" :title="$title ?? __('client.routes.index.meta_title')" :description="$description ?? ''">
    <section class="ksb-hero ksb-section-hero relative z-elevated px-4">
        ...
        <div class="container relative mx-auto max-w-7xl">
            <div class="max-w-3xl text-white">
                <h1 class="ksb-text-balance font-display text-4xl font-extrabold leading-tight sm:text-5xl md:text-6xl">
                    {{ __('client.routes.index.hero_title') }}
                </h1>
                <p class="mt-4 max-w-2xl text-base leading-8 text-slate-100/90 md:text-lg">
                    {{ __('client.routes.index.hero_subtitle') }}
                </p>
            </div>

            <div class="ksb-hero-search mt-9 lg:mt-10">
                ...
                <div class="flex gap-2 overflow-x-auto pb-2">
                    ...
                    @foreach ($quickRouteSuggestions->take(6) as $suggestion)
                        <a href="{{ route('client.routes.show', ['slug' => $suggestion->slug]) }}"
                            class="inline-flex shrink-0 items-center gap-2 rounded-sm border border-white/15 bg-white/10 px-3 py-2 text-sm font-semibold text-white transition hover:bg-white/20">
    ...
    <section class="ksb-section px-4">
        <div class="container mx-auto max-w-7xl">
            ...
            @if (isset($provinces) && $provinces->isNotEmpty())
                <div class="mb-8 flex gap-2 overflow-x-auto pb-2">
                    <a href="{{ route('client.routes.index') }}"
                        class="shrink-0 rounded-sm border px-4 py-2 text-sm font-semibold transition {{ !$selectedProvince ? 'border-primary-600 bg-primary-600 text-white' : 'border-slate-200 bg-white text-slate-600 hover:border-primary-300 hover:bg-primary-50' }}">
                        <i class="fa-solid fa-globe mr-1"></i>
                        {{ __('client.routes.index.all_provinces', ['default' => 'Tất cả']) }}
                    </a>
                    @foreach ($provinces as $province)
                        <a href="{{ route('client.routes.index', ['province' => $province->id]) }}"
                            class="shrink-0 rounded-sm border px-4 py-2 text-sm font-semibold transition {{ $selectedProvince == $province->id ? 'border-primary-600 bg-primary-600 text-white' : 'border-slate-200 bg-white text-slate-600 hover:border-primary-300 hover:bg-primary-50' }}">
                            {{ $province->name }}
                        </a>
                    @endforeach
                </div>
                            ...
                            <div class="flex min-w-0 flex-col justify-between gap-5 p-5">
                                <div>
                                    ...
                                    <div class="mt-4 grid gap-3 text-sm text-slate-500 sm:grid-cols-3">
                                        ...
                                        <span class="inline-flex items-center gap-2"><i class="fa-regular fa-clock text-primary-600"></i>{{ $route->duration ?? __('client.common.not_available') }}</span>
            ...
            @else
                <div class="rounded-sm border border-dashed border-amber-200 bg-white p-8 text-center text-sm text-slate-500 md:text-base">
